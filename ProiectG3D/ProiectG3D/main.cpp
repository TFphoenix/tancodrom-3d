...
int main()
{
	...
	Mesh monkeyMesh("./resources/monkey3.obj");
	Shader shader("./resources/testShader");
	Texture texture("./resources/bricks.jpg");
	Camera camera(glm::vec3(0, 0, -5), 70.0f, static_cast<float>(WINDOW_WIDTH) / WINDOW_HEIGHT, 0.01f, 1000.0f);
	Transform transform;
	...
	while (!display.IsClosed())
	{
		...
		transform.GetScale().y = sinf(counter/2);
		transform.GetScale().z = sinf(counter/2);
